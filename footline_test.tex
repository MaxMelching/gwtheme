...
\tikzset{
    fillsquare1/.style={
        ...
        % CaltechDarkBlue!90!CaltechDarkGrey,  % Slightly darker version of background
...
{\rmfamily\LARGE\mdseries
\color{CaltechDarkGrey}  % The more I look at it, the more I like it
% \color{CaltechDarkBlue!80}
This is some rather length introductory text}

\begin{tikzpicture}[overlay,remember picture]%
    \clip (current page.south west) -- +(0, 0.85\MeanFootHeight) -- +(\paperwidth, 1.15\MeanFootHeight) -- (current page.south east) -- cycle;
    % TODO: could also do clipping just on sectionpage, where we then have
    % the other colored box with same angle on top of the frame. And on the
    % regular frames, we have straight line
    ...
    % -- Testing out fontcolor
    % \node[shift={(0, -0.1\paperwidth)}] at (current page.north) {\LARGE\color{CaltechDarkGrey}Test};
    ...
    % \foreach \i in {0, 2, 4, ..., 8} {  % 8 must be set manually
    \pgfmathsetmacro{\LastIndex}{\paperwidth/(2*\MeanFootHeight)}
